...
| # | 항목 | 상태 |
|---|---|---|
| 18 | `content_assets`/`listings`/`listing_events` + 등록 엔진 | ✅ 완료 |
| 19 | 가격·재고 감시 (마진 감시 판정) | ✅ 완료 |
| 20 | `order_items`/`shipments`/`returns`/`refunds`/`settlements`/`expenses`/`profit_daily` | ✅ 완료 |
| 21 | 실현 손익 (예상 vs 실현, 오차 원인, 비용 워터폴) | ✅ 완료 |
| 22 | CS 초안 · PII 마스킹 (S11) | ⬜ 미착수 |
| 23 | 결제 승인 복구 (S3) | ⏸ 확인 대기 |

**P2에서 실제로 바뀐 것**

- **등록 엔진** (`lib/listing-engine.ts`) — `published`로 가는 유일한 경로는 `approved → publishing → published`다. `draft`나 `pending_approval`에서 곧바로 등록할 수 없고, 승인을 받아도 `publishing`을 지나야 한다(채널 API가 실제로 성공했는지 확인하기 위해). 테스트로 고정했다.
- **등록 3중 관문** (`POST /api/listings/[id]/publish`) — ① 상태 전이 규칙 ② 사전 조건 재확인 ③ 안전 게이트. 사전 조건은 승인 레코드·Bucky 추천·하드블록 0건·제목/설명 존재·**콘텐츠 사람 검수**·등록가·**등록 직전 마진 재확인**을 본다. 승인 시점 이후 소싱가가 올라 마진이 무너졌으면 막는다 — 승인 하나만 보고 등록하면 안 되는 이유다.
- **수익 엔진** (`lib/profit-engine.ts`, `GET /api/profit`) — 지시서 §18 "수익은 주문·정산·비용에서 계산한다"를 그대로 구현했다.
  - **실현 매출은 정산 실입금액만 인정한다.** 주문 금액(팔린 값)과 정산액(들어온 값)은 채널 수수료·프로모션 분담 때문에 다르다. `expected`·`disputed` 정산은 제외한다 — "줄 예정"은 들어온 돈이 아니다.
  - **결제 미승인 주문은 매출에서 제외한다.** Toss 승인이 없어 `payment_status='unconfirmed'`인 주문을 매출로 잡으면 가짜 수익이 된다.
  - 오차 원인 분석(환불·환차손·광고비·수수료·미증빙 비용), 비용 워터폴(마지막 잔액 = 실현 순이익), 파이프라인 전환율(이전 단계 0이면 `null`), 상품별 기여도 순위.
  - `/api/profit`은 `profit_daily` 캐시를 읽지 않고 원본에서 계산한다. 캐시가 틀렸을 때 화면이 틀린 숫자를 보여주면 안 된다.
- **마진 감시** — 품절 또는 적자면 자동 일시중지(지시서 §7 "안전한 자동 일시중지는 허용"), 기준 미달은 critical, 5%p 이상 하락은 warning.
- **가격 변경 판정** — 한도 안은 자동, 밖은 승인 필요. 다만 마진 기준을 깨는 변경은 **승인 여부와 무관하게 거부**한다.

**완료 기준 대비 현황**: 승인 전 외부 등록 미실행 ✅ (3중 관문 + 테스트) · 수익이 주문·정산·비용에서 산출 ✅ · CS·PII ⬜ · 결제 복구 ⏸
...
|---|---|---|
| 24 | 성과 기반 배정 | ✅ 완료 |
| 25 | 제한적 자동 승인 확대 (승격/강등 판정) | ✅ 완료 |
| 26 | 실패 사례 환류 | ✅ 완료 |

**P3에서 실제로 바뀐 것** (`lib/autonomy-tuning.ts`)

- **액션별 자율성 상한** — `publish_listing`·`payment`·`refund`·`send_customer_notice`는 `never`다. **어떤 실적을 내도 자동화되지 않는다** (실행 10,000회 전승이어도 승격 안 됨을 테스트로 고정). 정의되지 않은 신규 액션은 보수적으로 `manual`.
- **승격 조건** — 표본(20/100회) + 성공률(90%/97%) + 되돌림률(10%/2%) + 마지막 실패 이후 연속 실행(10/50회) + **실손실 0원**. 실손실이 1원이라도 있으면 확대하지 않는다.
  - 되돌림률을 따로 본 이유: 성공했지만 사람이 뒤집었다면 판단이 틀린 것이다. 성공률만으로는 안 잡힌다.
  - **승격은 항상 소유자 승인을 거친다.** 시스템이 스스로 권한을 넓히면 가드레일이 의미를 잃는다.
- **강등** — 승격과 달리 **즉시 적용**한다(위험을 줄이는 방향이라 승인을 기다릴 이유가 없다). 실손실이 나면 한 단계가 아니라 `manual`까지 내린다 — 한 단계만 내리면 자동 실행 경로가 남는다.
- **실패 환류** — 재시도로 해결되는 오류(TIMEOUT·NETWORK)와 그렇지 않은 오류(INVALID·PERMISSION)를 구분해 조치를 달리 제안한다. 스키마 오류를 계속 재시도하면 비용만 든다.
