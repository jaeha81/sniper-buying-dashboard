...
> **수익 증대 / 손실 없음** — 이것이 유일한 목표다.

- **에이전트 완전 자동화** — 수집·계산·경보·태스크 생성은 자동으로 실행
- **손실 방지 게이트 내장** — AI 오류·환율 급변·통관 리스크가 손실로 이어지기 전에 시스템이 차단
- **사용자 브리핑 & 검수** — 에이전트가 자동으로 돌지만 사용자는 Slack 브리핑을 통해 언제든 확인·개입 가능
- **손실 유발 구현 금지** — 검증되지 않은 데이터가 판매·구매 실행으로 이어지는 경로는 만들지 않는다
