...
## 🤖 완전자율 에이전트 운영 (Autonomy Engine)

5개 전문 에이전트(상품 발굴 / 마진·가격 / 주문 처리 / 리스크 / 운영 지휘)가 시스템을 스캔해
태스크를 생성하고, **자율성 정책 엔진**이 가드레일 내에서 사람 승인 없이 즉시 실행합니다.

- **3단계 자율 레벨**: 수동(manual) → 반자율(assisted, 기본) → 완전자율(autopilot)
- **자동 실행 예시**: 저마진 상품 방어적 일시중지, 한도 내 가격 자동 조정, 고득점 상품 자동 승인
- **가드레일**: 일일 실행 한도, 가격 변동 폭 제한, 자동 승인 스코어 임계값, 긴급 정지 킬스위치
- **감사 추적**: 모든 자율 실행은 실행 주체·사유·결과가 `agent_tasks`에 기록되고 Slack으로 통보
- **제어판**: `/admin/agent-command` 상단에서 레벨 전환·킬스위치·이력 확인

자세한 설계와 액션별 자율 실행 매트릭스는 [`docs/AUTONOMY_ENGINE.md`](docs/AUTONOMY_ENGINE.md) 참고.
